    ...
    public static class DomainDalMapper
    {
        ...
        public static Contact ToDomain(this DalContact dalContact)
        {
            return dalContact == null ? null : new Contact(
                ...
                dalContact.Phones
                    .Select(
                        x => new Phone(x.PhoneNo,
                        Enum.Parse<PhoneType>(x.Type))
                    ).ToList(),
                dalContact.Addresses
                    .Select(x => new Address(x))
                    .ToList(),
                dalContact.Emails
                    .Select(x => new Email(x))
                    .ToList(),
                dalContact.Categories
                    .Select(x => new Category(x))
                    .ToList(),
                dalContact.Tags
                    .Select(x => new Tag(x))
                    .ToList(),
                dalContact.CustomFields
